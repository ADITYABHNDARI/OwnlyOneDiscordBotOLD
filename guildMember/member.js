...
async function memberAdd (member) {
  const welcomeChannel = member.guild.channels.cache.find(channel => channel.topic && channel.topic.startsWith('|welcome|'));
...
async function memberLeave (member, channelCache) {
  const goodbyeChannel = channelCache.find(channel => channel.topic && channel.topic.startsWith('|goodbye|'));
